...
# content with security
   ...
   #$page->addContainer('mailFormId','mailForm',  '');
    ...
    $objTable->addRow();
    $objTable->addField('xxx','mailCompactLeft',$page->getContainer('mailFormId','xxxxx',  'List of mails'));
    $objTable->addField('xxx','mailCompactRight',$page->getContainer('contantArea','yyyyy',  'Mail preview'));
    $page->addContainer('contentDiv','mailForm', $objTable->getTable());

   # $page->positionContainerAbsolut("contentDiv", "CENTER", "CENTER", 10, 10, 100,10);
   $page->positionContainerAbsolutMax("contentDiv", "TOP", "LEFT", 10, 10, 100,10);

   $page->setBodyOnLoad("getMailsOnLoad(\"./ajaxLoadExistingMailCompact.pl?category=unread&\", \"./ajaxLoadExistingMailCompact.pl?category=read\", \"./ajaxLoadMail.pl\", \"mailFormId\");");
