...
$objPage->addStyleSheet($objConfig->cssPath() . 'iPhoneStyle.css');


#$objPage->addJavaScript($objConfig->jsPath() . 'classes.js');
$objPage->addJavaScript($objConfig->jsPath() . 'ajax.js');
$objPage->addJavaScript($objConfig->jsPath() . 'iPhoneObject.js');
...
if(($objABL->checkPassword($v_u, $v_p, $v_x) eq 1) || ($objABL->checkLogin($v_u, $v_s) eq 1) ){
    ...
    $objPage->addContainer('iPhone','iPhone','iPhone');
    $objPage->setGlobalVariables($v_s, $v_u);
    $objPage->setBodyOnLoad("iPhoneObject.initialize();");
